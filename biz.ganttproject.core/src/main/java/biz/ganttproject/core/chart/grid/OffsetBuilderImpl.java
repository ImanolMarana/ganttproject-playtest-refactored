...
//  }
  ...
  do {
    ...
    Date endDate = concreteTimeUnit.adjustRight(currentDate);
    int offsetEnd = calculateTopOffsetEnd(endDate, bottomOffsets, offsetLookup, lastBottomOffset, baseUnitWidth);
...
private int calculateTopOffsetEnd(Date endDate, List<Offset> bottomOffsets, OffsetLookup offsetLookup, 
                                    int lastBottomOffset, int baseUnitWidth) {
  int bottomOffsetLowerBound = offsetLookup.lookupOffsetByEndDate(endDate, bottomOffsets);

  if (bottomOffsetLowerBound >= 0) {
    return bottomOffsets.get(bottomOffsetLowerBound).getOffsetPixels();
  } else if (-bottomOffsetLowerBound > bottomOffsets.size()) {
    return lastBottomOffset + 1;
  } else {
    return calculateOffsetEndBeyondBottomOffsets(endDate, bottomOffsets, bottomOffsetLowerBound, baseUnitWidth);
  }
}

private int calculateOffsetEndBeyondBottomOffsets(Date endDate, List<Offset> bottomOffsets, 
                                                  int bottomOffsetLowerBound, int baseUnitWidth) {
  ...
  return ubEndPixel + counter.run(ubEndDate, endDate).getLength() * baseUnitWidth;
}
