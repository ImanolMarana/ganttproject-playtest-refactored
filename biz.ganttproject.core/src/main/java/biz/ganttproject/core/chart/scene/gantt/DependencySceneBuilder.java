...
public class DependencySceneBuilder<T extends IdentifiableRow, D extends BarChartConnector<T, D>> {
  ...
  private Connector createConnector(D connector, BarChartActivity<T> dependant, BarChartActivity<T> dependee,
    ...
    final int yDantEntry = calculateYDantEntry(endArrrow, ysign, dependantRectangle);
    final Dimension dependantDirection = myTaskApi.getUnitVector(dependant, connector);
    int xDantEntry = calculateXDantEntry(dependant, dependantRectangle, dependantDirection, endArrrow);
    ...
    if (myTaskApi.isMilestone(dependee.getOwner()) && xDantEntry == dependeeRectangle.getMiddleX()) {
      ...
      xDeeExit = calculateXDeeExit(dependeeDirection, dependeeRectangle);
    }
  ...
  private int calculateYDantEntry(ConnectorEndArrow endArrrow, int ysign, Canvas.Polygon dependantRectangle) {
    if (endArrrow == ConnectorEndArrow.VERTICAL) {
      return (ysign > 0 ? dependantRectangle.getBottomY() : dependantRectangle.getTopY());
    } else {
      return dependantRectangle.getMiddleY();
    }
  }

  private int calculateXDantEntry(BarChartActivity<T> dependant, Canvas.Polygon dependantRectangle, Dimension dependantDirection, ConnectorEndArrow endArrrow) {
    if (myTaskApi.isMilestone(dependant.getOwner())) {
      return dependantRectangle.getMiddleX();
    } else if (dependantDirection == Connector.Vector.WEST) {
      return endArrrow == ConnectorEndArrow.VERTICAL ? dependantRectangle.getLeftX() + 3 : dependantRectangle.getLeftX();
    } else if (dependantDirection == Connector.Vector.EAST) {
      return endArrrow == ConnectorEndArrow.VERTICAL ? dependantRectangle.getRightX() - 3 : dependantRectangle.getRightX();
    } else {
      return dependantRectangle.getMiddleX();
    }
  }

  private int calculateXDeeExit(Dimension dependeeDirection, Canvas.Polygon dependeeRectangle) {
    if (dependeeDirection == Connector.Vector.WEST) {
      return dependeeRectangle.getLeftX();
    } else if (dependeeDirection == Connector.Vector.EAST) {
      return dependeeRectangle.getRightX();
    } else {
      return dependeeRectangle.getMiddleX();
    }
  }
