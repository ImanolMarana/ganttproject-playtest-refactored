...
public class TaskActivitySceneBuilder<T extends IdentifiableRow, A extends BarChartActivity<T>> {
  ...
  private void processRegularActivity(int rowNum, A activity, OffsetList offsets, List<Polygon> polygons) {
    ...
    Canvas.Polygon resultShape = createActivityShape(container, nextTask, nextBounds, nextLength, topy);
  ...
  private Canvas.Polygon createActivityShape(Canvas container, T nextTask, java.awt.Rectangle nextBounds,
      int nextLength, int topy) {
    final int rectTopY =
        topy + nextBounds.height / 2 - getRectangleHeight() / 2 + myLabelsRenderer.getRectMidOffset()
            + myStyle.marginTop;
    if (myTaskApi.isMilestone(nextTask)) {
      return createMilestoneShape(container, nextBounds, rectTopY, nextLength);
    } else {
      return createRectangleShape(container, nextTask, nextBounds, nextLength, rectTopY);
    }
  }

  private Canvas.Polygon createMilestoneShape(Canvas container, java.awt.Rectangle nextBounds, int rectTopY,
      int nextLength) {
    Canvas.Rectangle rect =
        container.createDetachedRectangle(nextBounds.x, rectTopY, nextLength, getRectangleHeight());
    int rectHeight = rect.getHeight();
    int rectHalf = rectHeight / 2;
    int middleX = rect.getLeftX() + 3;
  ...
  private Canvas.Polygon createRectangleShape(Canvas container, T nextTask,
      java.awt.Rectangle nextBounds, int nextLength, int rectTopY) {
    Canvas.Rectangle nextRectangle =
        container.createRectangle(nextBounds.x, rectTopY, nextLength, getRectangleHeight());
    if (myTaskApi.hasNestedTasks(nextTask) || myTaskApi.isProjectTask(nextTask)) {
      String style =
          myTaskApi.isProjectTask(nextTask) ? "task.projectTask" : "task.supertask";
      ...
      applySimpleTaskStyle(nextRectangle);
    }
    return nextRectangle;
  }

  private void applySimpleTaskStyle(Canvas.Rectangle nextRectangle) {
    if (myTaskApi.isFirst(activity) && myTaskApi.isLast(activity)) {
