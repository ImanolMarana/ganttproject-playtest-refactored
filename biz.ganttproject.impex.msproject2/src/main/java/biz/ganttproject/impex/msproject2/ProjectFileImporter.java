...
class ProjectFileImporter {
  ...
  private void importDependencies(ProjectFile pf, Map<Integer, GanttTask> foreignId2nativeTask) {
    ...
    try {
      for (Task t : pf.getTasks()) {
        importDependencies(pf, foreignId2nativeTask, t);
      }
    } finally {
  ...
  private void importDependencies(ProjectFile pf, Map<Integer, GanttTask> foreignId2nativeTask, Task t) {
    if (t.getPredecessors() == null) {
      return;
    }
    for (Relation r : t.getPredecessors()) {
      importDependency(pf, foreignId2nativeTask, t, r);
    }
  }
  
  private void importDependency(ProjectFile pf, Map<Integer, GanttTask> foreignId2nativeTask, Task t, Relation r) {
