...
public class GanttCSVExport {
  ...
  private void writeResources(SpreadsheetWriter writer) throws IOException {
    ...
    for (HumanResource p : myHumanResourceManager.getResources()) {
      writeResourceData(writer, projectRoles, customPropDefs, p);
    }
  }

  private void writeResourceData(SpreadsheetWriter writer, Set<Role> projectRoles,
      List<CustomPropertyDefinition> customPropDefs, HumanResource p) throws IOException {
    for (Map.Entry<String, BooleanOption> entry : myCsvOptions.getResourceOptions().entrySet()) {
      if (!entry.getValue().isChecked()) {
        continue;
      }
      ...
      if (defaultColumn == null) {
        ...
        writeResourceColumn(writer, projectRoles, p, defaultColumn);
      }
    }
    CSVExportKt.writeCustomPropertyValues(writer, customPropDefs, p.getCustomProperties());
  }

  private void writeResourceColumn(SpreadsheetWriter writer, Set<Role> projectRoles, HumanResource p,
      ResourceDefaultColumn defaultColumn) throws IOException {
    switch (defaultColumn) {
      case NAME:
