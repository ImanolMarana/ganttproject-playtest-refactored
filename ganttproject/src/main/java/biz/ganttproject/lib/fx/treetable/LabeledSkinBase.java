...
public abstract class LabeledSkinBase<C extends Labeled> extends SkinBase<C> {
  ...
  protected void layoutLabelInArea(double x, double y, double w, double h, Pos alignment) {
    ...
    // Layout the graphic and text
    layoutContent(x, y, w, h, contentDisplay, ignoreGraphic, ignoreText);

    // Handle text clipping if necessary
    handleTextClipping();
  }

  private void layoutContent(double x, double y, double w, double h, ContentDisplay contentDisplay,
                              boolean ignoreGraphic, boolean ignoreText) {
    // Compute dimensions and positions for graphic and text
    double[] graphicDimensions = calculateGraphicDimensions(w, h, ignoreGraphic, ignoreText);
    double graphicWidth = graphicDimensions[0];
    double graphicHeight = graphicDimensions[1];

    double[] textDimensions = calculateTextDimensions(w, h, ignoreGraphic, ignoreText);
    double textWidth = textDimensions[0];
    double textHeight = textDimensions[1];

    double gap = (ignoreText || ignoreGraphic) ? 0 : getSkinnable().getGraphicTextGap();
    ...
    double contentX = calculateContentX(x, w, contentWidth, hpos);
    double contentY = calculateContentY(y, h, contentHeight, vpos);

    // Position graphic and text
    positionGraphicAndText(contentX, contentY, contentWidth, contentHeight, graphicWidth,
        graphicHeight, textWidth, textHeight, gap, contentDisplay);
  }

  private double[] calculateGraphicDimensions(double w, double h, boolean ignoreGraphic, boolean ignoreText) {
    final Labeled labeled = getSkinnable();
    double graphicWidth;
    ...
    return new double[]{graphicWidth, graphicHeight};
  }

  private double[] calculateTextDimensions(double w, double h, boolean ignoreGraphic, boolean ignoreText) {
    double textWidth;
    double textHeight;
  ...
  private double calculateContentX(double x, double w, double contentWidth, HPos hpos) {
    if (hpos == HPos.LEFT) {
      return x;
    } else if (hpos == HPos.RIGHT) {
      return x + (w - contentWidth);
    } else {
      // TODO Baseline may not be handled correctly
      // may have been CENTER or null, treat as center
      return (x + ((w - contentWidth) / 2.0));
    }
  }

  private double calculateContentY(double y, double h, double contentHeight, VPos vpos) {
    if (vpos == VPos.TOP) {
      return y;
    } else if (vpos == VPos.BOTTOM) {
      return (y + (h - contentHeight));
    } else {
      // TODO Baseline may not be handled correctly
      // may have been CENTER, BASELINE, or null, treat as center
      return (y + ((h - contentHeight) / 2.0));
    }
  }

  private void positionGraphicAndText(double contentX, double contentY, double contentWidth,
                                       double contentHeight, double graphicWidth, double graphicHeight,
                                       double textWidth, double textHeight, double gap, ContentDisplay contentDisplay) {
    final Labeled labeled = getSkinnable();
    ...
    if ((!isIgnoreGraphic() || !isIgnoreText()) && !text.isManaged()) {
    ...
    if (isIgnoreGraphic() && isIgnoreText()) {
      ...
      positionText(contentX, contentY, textWidth);
    } else if (isIgnoreText()) {
      ...
      positionMnemonic(contentX, contentY);
    } else {
      ...
      positionMnemonic(textX, textY);
      graphic.relocate(snapPositionX(graphicX), snapPositionY(graphicY));
    }
  }

  private void positionText(double contentX, double contentY, double textWidth) {
    text.relocate(snapPositionX(contentX), snapPositionY(contentY));
  ...
  private void positionMnemonic(double contentX, double contentY) {
    if (containsMnemonic && (mnemonicPos != null)) {
  ...
  private void handleTextClipping() {
    if ((text != null) &&
      ...
      if (getSkinnable().getEffectiveNodeOrientation() == NodeOrientation.LEFT_TO_RIGHT) {
