...
//            }
  ...
  void updateTableColumnHeaders() {
    updateColumnsFromTableOrColumn();

    if (getColumns().isEmpty()) {
        switchToSingleColumnHeader();
    } else {
        updateExistingHeaders();
    }
...
private void updateColumnsFromTableOrColumn() {
...
private void switchToSingleColumnHeader() {
    disposeExistingHeaders();

    NestedTableColumnHeader parentHeader = getParentHeader();
    if (parentHeader != null) {
        replaceWithColumnHeaderInParent(parentHeader);
    } else {
        columnHeaders.clear();
    }
}

private void disposeExistingHeaders() {
    for (int i = 0; i < getColumnHeaders().size(); i++) {
        TableColumnHeader header = getColumnHeaders().get(i);
        header.dispose();
    }
...
private void replaceWithColumnHeaderInParent(NestedTableColumnHeader parentHeader) {
...
private void updateExistingHeaders() {
    ...
    for (int i = 0; i < getColumns().size(); i++) {
        ...
        addNewHeader(oldHeaders, newHeaders, column);
    }

    columnHeaders.setAll(newHeaders);

    disposeOldHeaders(oldHeaders, newHeaders);
}

private void addNewHeader(List<TableColumnHeader> oldHeaders, List<TableColumnHeader> newHeaders, TableColumnBase<?, ?> column) {
    if (headerExists(oldHeaders, column)) {
        reuseExistingHeader(oldHeaders, newHeaders, column);
    } else {
        createNewHeader(newHeaders, column);
    }
}

private boolean headerExists(List<TableColumnHeader> oldHeaders, TableColumnBase<?, ?> column) {
    for (int j = 0; j < oldHeaders.size(); j++) {
        if (oldHeaders.get(j).represents(column)) {
            return true;
        }
    }
    return false;
...
private void reuseExistingHeader(List<TableColumnHeader> oldHeaders, List<TableColumnHeader> newHeaders, TableColumnBase<?, ?> column) {
    for (int j = 0; j < oldHeaders.size(); j++) {
...
private void createNewHeader(List<TableColumnHeader> newHeaders, TableColumnBase<?, ?> column) {
    newHeaders.add(createColumnHeader(column));
}

private void disposeOldHeaders(List<TableColumnHeader> oldHeaders, List<TableColumnHeader> newHeaders) {
...
private void applyCssToHeaders() {
