...
public class GanttGraphicArea extends ChartComponentBase implements GanttChart, CustomPropertyListener,
  ...
  private List<Action> createToggleHolidayAction(int x) {
    ...
    CalendarChartItem calendarChartItem = (CalendarChartItem) chartItem;
    Date date = calendarChartItem.getDate();
    ...
    if ((dayMask & GPCalendar.DayMask.WEEKEND) != 0) {
      addWeekendActions(result, date, dayMask, event);
    }
  ...
  private void addWeekendActions(List<Action> result, Date date, int dayMask, CalendarEvent event) {
