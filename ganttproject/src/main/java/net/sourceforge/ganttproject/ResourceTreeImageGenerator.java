...
class ResourceTreeImageGenerator {
  ...
  private void printResources(Graphics g, int width) {
    ...
    for (Iterator<HumanResource> user = users.iterator(); user.hasNext();) {
      HumanResource hr = user.next();
      y = printResourceName(g, hr, y, width, nameLineHeight, nameLinePadding, isOddRow);
      isOddRow = !isOddRow;
      y = printAssignedTasks(g, hr, y, width, nameLineHeight, nameLinePadding, isOddRow);
      isOddRow = !isOddRow;
    }
  }

  private int printResourceName(Graphics g, HumanResource hr, int y, int width, int nameLineHeight, int nameLinePadding, boolean isOddRow) {
  ...
  private int printAssignedTasks(Graphics g, HumanResource hr, int y, int width, int nameLineHeight, int nameLinePadding, boolean isOddRow) {
    ResourceAssignment[] assignments = hr.getAssignments();
    if (assignments != null) {
      for (int i = 0; i < assignments.length; i++) {
        if (isAssignmentVisible(assignments[i])) {
