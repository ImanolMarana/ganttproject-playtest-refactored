...
public class GanttChartSceneBuilder {
  ...
  private void renderBaseline(ITaskSceneTask t, int rowNum, OffsetList defaultUnitOffsets) {
        ...
        GanttPreviousStateTask taskBaseline = findMatchingBaselineTask(t, baseline);
        ...
        List<String> styles = determineBaselineStyles(t, endDate);
        List<ITaskActivity<ITaskSceneTask>> baselineActivities = createBaselineActivities(t, startDate, endDate, alg);
    
        List<Polygon> baselineRectangles = myBaselineActivityRenderer.renderActivities(rowNum, baselineActivities,
                defaultUnitOffsets);
        styleBaselineRectangles(baselineRectangles, styles);
      }
    
      private GanttPreviousStateTask findMatchingBaselineTask(ITaskSceneTask task, List<GanttPreviousStateTask> baseline) {
        for (GanttPreviousStateTask taskBaseline : baseline) {
            if (taskBaseline.getId() == task.getRowId()) {
                return taskBaseline;
            }
        }
        return null;
      ...
      private List<String> determineBaselineStyles(ITaskSceneTask task, Date baselineEndDate) {
        List<String> styles = new ArrayList<>();
        if (task.isMilestone()) {
            styles.add("milestone");
        }
        if (baselineEndDate.compareTo(task.getEnd().getTime()) < 0) {
            styles.add("later");
        } else {
            styles.add("earlier");
        }
      ...
      private List<ITaskActivity<ITaskSceneTask>> createBaselineActivities(ITaskSceneTask task, Date startDate, Date endDate, TaskActivitiesSceneAlgorithm alg) {
        List<ITaskActivity<ITaskSceneTask>> baselineActivities = new ArrayList<>();
        if (task.isMilestone()) {
            baselineActivities.add(
                    new TaskSceneMilestoneActivity(task, startDate, endDate, input.createLength(1))
            );
        } else {
            alg.recalculateActivities(task, baselineActivities, startDate, endDate);
        }
        return baselineActivities;
      }
    
      private void styleBaselineRectangles(List<Polygon> baselineRectangles, List<String> styles) {
