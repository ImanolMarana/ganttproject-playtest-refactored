...
public class RssParser {
  ...
  private boolean isApplicableToVersion(Node item, String version, String build) throws XPathExpressionException {
        ...
        return !hasVersionRestriction(categories, version, build);
    }

    private boolean hasVersionRestriction(NodeList categories, String version, String build) throws XPathExpressionException {
        for (int i = 0; i < categories.getLength(); i++) {
            ...
            if (category.startsWith("__version") && compareCategory(category, "version", version)) {
                return true;
            }
            if (category.startsWith("__build") && compareCategory(category, "build", build)) {
                return true;
            }
        }
        return false;
