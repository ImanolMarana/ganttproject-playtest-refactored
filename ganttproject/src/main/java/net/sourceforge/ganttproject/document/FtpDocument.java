...
public class FtpDocument extends AbstractURLDocument {
  ...
  FtpDocument(String urlAsString, StringOption ftpUser, StringOption ftpPassword) {
        ...
        try {
            myURI = createUriWithUserInfo(urlAsString, ftpUser, ftpPassword);
            urlAsString = myURI.toString();
            myURI.toURL().openConnection().connect();
        } catch (URISyntaxException e) {
            handleException(e, urlAsString);
    ...
    private URI createUriWithUserInfo(String urlAsString, StringOption ftpUser, StringOption ftpPassword) throws URISyntaxException {
        ...
        if (userInfo == null || EMPTY_STRING.equals(userInfo)) {
            ...
            return new URI("ftp", buf.toString(), url.getHost(), url.getPort(), url.getPath(), url.getQuery(),
                    url.getFragment());
        } else {
            return url;
        }
    ...
    private void handleException(Exception e, String urlAsString) {
        if (!GPLogger.log(e)) {
