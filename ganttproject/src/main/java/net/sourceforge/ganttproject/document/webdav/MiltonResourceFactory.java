...
public class MiltonResourceFactory {
  ...
  static ProxyDetails getProxyDetails(StringOption proxyOption) {
        ...
        if ("system".equalsIgnoreCase(url)) {
            ...
            result = parseProxyUrl(url);
        }
        return result;
    }
    
    private static ProxyDetails parseProxyUrl(String url) {
        ProxyDetails result = new ProxyDetails();
        ...
        try {
            ...
            parseUserInfo(result, parsedUrl);
    ...
    private static void parseUserInfo(ProxyDetails result, URL parsedUrl) {
