...
public class WebPublisher {
  ...
  public void run(final File[] exportFiles, final DocumentManager.FTPOptions options) {
        ...
        Job startingJob = createStartingJob(exportFiles, options, monitor);
        startingJob.setProgressGroup(monitor, 0);
        startingJob.schedule();
    }
    
    private Job createStartingJob(final File[] exportFiles, final DocumentManager.FTPOptions options, final IProgressMonitor monitor) {
        return new Job("starting") {
            @Override
            protected IStatus run(IProgressMonitor monitor) {
                ...
                try {
                    ...
                    scheduleTransferJobs(exportFiles, monitor, ftp);
                    scheduleFinishingJob(monitor, ftp);
                } catch (IOException e) {
                    handleIOException(e);
            ...
            private void scheduleTransferJobs(final File[] exportFiles, final IProgressMonitor monitor, final Ftp ftp) throws InterruptedException {
            ...
            private void scheduleFinishingJob(final IProgressMonitor monitor, final Ftp ftp) throws InterruptedException {
                Job finishingJob = createFinishingJob(monitor, ftp);
                finishingJob.setProgressGroup(monitor, 0);
                finishingJob.schedule();
                finishingJob.join();
            }

            private Job createFinishingJob(final IProgressMonitor monitor, final Ftp ftp) {
                return new Job("finishing") {
                    @Override
                    protected IStatus run(IProgressMonitor monitor) {
                        monitor.done();
                        try {
