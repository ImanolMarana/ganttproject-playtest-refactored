...
public class InterfaceOptionPageProvider extends OptionPageProviderBase {
  ...
  private static Pair<Boolean, File> checkLocale(Locale l) {
    ...
    if (!isValidExtDir(extDir)) {
        return Pair.create(Boolean.FALSE, null);
    }
    return checkAndInstallGalicianLocale(extDir);
}

private static boolean isValidExtDir(File extDir) {
    return extDir.exists() && extDir.isDirectory();
}

private static Pair<Boolean, File> checkAndInstallGalicianLocale(File extDir) {
    if (extDir.canWrite()) {
