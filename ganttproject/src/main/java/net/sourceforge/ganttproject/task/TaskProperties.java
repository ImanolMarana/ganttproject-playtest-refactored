...
public class TaskProperties {
  ...
  public static void parseDependency(String depSpec, final Task successor, Function<Integer, Task> taskIndex,
          ...
          if (posDash < 0) {
              handleSimpleDependency(depSpec, successor, taskIndex, out, predecessorId);
              return;
          }
          ...
          if (depSpec.length() == posDash + 3) {
              handleDependencyWithConstraint(depSpec, successor, out, predecessorId, taskMgr, predecessor, depType);
              return;
          }
          
          handleDependencyWithLag(depSpec, successor, out, posDash, predecessorId, taskMgr, predecessor, depType);
      }
  
      private static void handleDependencyWithLag(String depSpec, final Task successor,
              Map<Integer, Supplier<TaskDependency>> out, int posDash, final Integer predecessorId,
              final TaskManager taskMgr, final Task predecessor, final ConstraintType depType) {
      ...
      private static void handleDependencyWithConstraint(String depSpec, final Task successor,
              Map<Integer, Supplier<TaskDependency>> out, final Integer predecessorId, final TaskManager taskMgr,
              final Task predecessor, final TaskDependencyConstraint constraint) {
      ...
      private static void handleSimpleDependency(String depSpec, final Task successor, Function<Integer, Task> taskIndex,
              Map<Integer, Supplier<TaskDependency>> out, final Integer predecessorId) {
          final Task predecessor = taskIndex.apply(predecessorId);
          if (predecessor == null) {
              throw new IllegalArgumentException(String.format("Can't find task with ID=%s", depSpec));
          }
          out.put(predecessorId, () -> {
              if (successor.getManager().getDependencyCollection().canCreateDependency(successor, predecessor)) {
                  return successor.getManager().getDependencyCollection().createDependency(successor, predecessor);
              }
