...
public class ExporterToHTML extends StylesheetExporterBase {
  ...
  private void copyImages(File outputFile) throws IOException {
    ...
    if (imagesDir != null && imagesDir.isDirectory() && imagesDir.exists()) {
      ...
      if (lof.length != 0) {
        ...
        if (resultImagesDir.mkdir()) {
          copyFilesToDir(lof, resultImagesDir);
        }
      }
    }
  }

  private void copyFilesToDir(File[] lof, File resultImagesDir) throws IOException {
    for (File nextInFile : lof) {
      if (nextInFile.isDirectory()) {
        continue;
      }
      File outFile = new File(resultImagesDir, nextInFile.getName());
      outFile.createNewFile();
      copyFileContent(nextInFile, outFile);
    }
  }
  
  private void copyFileContent(File sourceFile, File destFile) throws IOException {
