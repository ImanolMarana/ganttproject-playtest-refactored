  ...
  private void writeTasks() throws Exception {
    ...
    TaskVisitor taskVisitor = new TaskVisitor() {
      ...
      protected String serializeTask(Task t, int depth) {
        boolean addEmptyRow = shouldAddEmptyRow(depth);
        if (addEmptyRow) {
        ...
        PdfPCell nameCell = createNameCell(t);
      ...
      private boolean shouldAddEmptyRow(int depth) {
        if (depth == 0) {
          boolean addEmptyRow = myPreviousChildTaskCount > 0;
          if (!addEmptyRow) {
            boolean hasNested = getProject().getTaskManager().getTaskHierarchy().hasNestedTasks(getProject().getTaskManager().getTaskById(1));
            if (hasNested) {
      ...
      private PdfPCell createNameCell(Task t) {
        if (myShowNotesOption.isChecked() && t.getNotes() != null && !"".equals(t.getNotes())) {
          return new PdfPCell(createNameCellContent(t));
        } else {
          return new PdfPCell(new Paragraph(t.getName(), getSansRegular(getFontSize())));
        }
      }
